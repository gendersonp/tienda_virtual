						...
						if(isset($_SESSION['arrCarrito'])){
							...
							for ($pr=0; $pr < count($arrCarrito); $pr++) {
								if($arrCarrito[$pr]['idproducto'] == $idproducto){
									...
									$subCant = $arrCarrito[$pr]['cantidad'] += $cantidad;

									if( $subCant > $arrCarrito[$pr]['stock']){
										$arrResponse = array("status" => false, "msg" => 'La cantidad tiene que ser menor a '. $arrCarrito[$pr]['stock']);
							...
							foreach ($_SESSION['arrCarrito'] as $producto)
							 {
								...
								if($producto['stock'] - $producto['cantidad'] == 0)
								{
								 $this->updateProducto2($producto['idproducto'], $producto['stock'] - $producto['cantidad'], 0);									
							    }
								else
								{
								 $this->updateProducto2($producto['idproducto'], $producto['stock'] - $producto['cantidad'], 1);
								}
							 }
		...
		public function contacto(){
			if($_POST){
				...
				if($userContact > 0){
					...
					$arrResponse = array('status' => false, 'msg' => "No es posible enviar su mensaje.");
				}
				echo json_encode($arrResponse,JSON_UNESCAPED_UNICODE);
