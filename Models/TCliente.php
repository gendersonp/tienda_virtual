...
trait TCliente{
	...
	public function updateProducto2(int $idproducto, int $stock, int $status){
		...
		$request = null;
		$this->con = new Mysql();
		$sql = "SELECT * FROM producto p WHERE idproducto =  $idproducto";
		$request = $this->con->select_all($sql);
		if($request!= null)
		{
			$sql = "UPDATE producto 
			...
			$arrResponse = array("status" => false, "msg" =>$sql. "--------". $request );
